...
-- Credit Card
INSERT INTO prices (coins_quantity, price, method_id)
SELECT 100, 1.99, pm.id_payment_method
FROM payment_methods pm
WHERE pm.payment_method='Credit Card'
  AND NOT EXISTS (
    SELECT 1 FROM prices p WHERE p.coins_quantity=100 AND p.method_id=pm.id_payment_method
  );
...
WHERE pm.payment_method='Credit Card'
  AND NOT EXISTS (
    SELECT 1 FROM prices p WHERE p.coins_quantity=500 AND p.method_id=pm.id_payment_method
  );
...
-- ============================
-- 3) Discounts (5)
--    Nota: columna se llama porcentaje_descuento
--    Hoy (Europe/Madrid): 2025-09-22
-- ============================

-- Pasado: Sep 1–7, 2025 (10%) para 500 coins de TODOS
INSERT INTO discounts (start_date, end_date, porcentaje_descuento, price_id)
SELECT '2025-09-01 00:00:00', '2025-09-07 23:59:59', 10, p.id_price
FROM prices p
WHERE p.coins_quantity = 500
  AND NOT EXISTS (
...
WHERE p.coins_quantity = 500 AND m.payment_method='Credit Card'
  AND NOT EXISTS (
    SELECT 1 FROM discounts d
    WHERE d.price_id = p.id_price
...
FROM prices p
JOIN payment_methods m ON m.id_payment_method = p.method_id
WHERE p.coins_quantity = 100 AND m.payment_method='PayPal'
  AND NOT EXISTS (
    SELECT 1 FROM discounts d
    WHERE d.price_id = p.id_price
...
FROM prices p
JOIN payment_methods m ON m.id_payment_method = p.method_id
WHERE p.coins_quantity = 100 AND m.payment_method='Bank Transfer'
  AND NOT EXISTS (
    SELECT 1 FROM discounts d
    WHERE d.price_id = p.id_price
...
FROM prices p
JOIN payment_methods m ON m.id_payment_method = p.method_id
WHERE p.coins_quantity = 100 AND m.payment_method='Credit Card'
  AND NOT EXISTS (
    SELECT 1 FROM discounts d
    WHERE d.price_id = p.id_price
